...
function render() {
  ...
  //Elshan wrote new mvm
  //Orxan generated t_mat for translate
  ...
  let mvm2;
  t_mat = translate(1.35,0,0);
  mvm2 = mult(mvm, t_mat);
  mvm2 = mult(mvm2, rotate(theta, rotationfactor));
  mvm2 = mult(mvm2, scalem(factorOfZoom, factorOfZoom, factorOfZoom));
  gl.uniformMatrix4fv(modelViewMatrix, false, flatten(mvm2));
  gl.drawArrays(gl.TRIANGLES, 0, points.length);

  
  let mvm3;
  t_mat2 = translate(2.1,0,0);
  mvm3 = mult(mvm, t_mat2);
  mvm3 = mult(mvm3, rotate(theta, rotationfactor));
  mvm3 = mult(mvm3, scalem(factorOfZoom, factorOfZoom, factorOfZoom));
  gl.uniformMatrix4fv(modelViewMatrix, false, flatten(mvm3));
  gl.drawArrays(gl.TRIANGLES, 0, points.length);
  
  let mvm4;
  t_mat3 = translate(2.9,0,0);
  mvm4 = mult(mvm, t_mat3);
  mvm4 = mult(mvm4, rotate(theta, rotationfactor));
  mvm4 = mult(mvm4, scalem(factorOfZoom, factorOfZoom, factorOfZoom));
