...
let t_mat3 = translate(3.2, 0, 0); // Initialize t_mat3
onload = () => {
...
function render() {
  ...
  //Javidan tried its own way to do rotation axes
  // sun
  ...
  // 1st planet
  let mvm2;
  let t_mat = translate(1.7, 0, 0);
  let s_mat = scalem(0.5, 0.5, 0.5); // Scaling factor for the 1st planet
  let rotationSpeed1 = 0.5; // Adjust the rotation speed for the 1st planet
  mvm2 = mult(mvm, t_mat);
  mvm2 = mult(mvm2, s_mat);
  mvm2 = mult(mvm2, rotate(theta * rotationSpeed1, rotationfactor));
  gl.uniformMatrix4fv(modelViewMatrix, false, flatten(mvm2));
  gl.drawArrays(gl.TRIANGLES, 0, points.length);

  // 2nd planet
  let mvm3;
  let t_mat2 = translate(2.6, 0, 0);
  let s_mat2 = scalem(0.3, 0.3, 0.3); // Scaling factor for the 2nd planet
  let rotationSpeed2 = 1.2; // Adjust the rotation speed for the 2nd planet
  mvm3 = mult(mvm, t_mat2);
  mvm3 = mult(mvm3, s_mat2);
  mvm3 = mult(mvm3, rotate(theta * rotationSpeed2, rotationfactor));
  gl.uniformMatrix4fv(modelViewMatrix, false, flatten(mvm3));
  gl.drawArrays(gl.TRIANGLES, 0, points.length);

  // 3rd planet
  let mvm4;
  let t_mat3 = translate(3.2, 0, 0);
  let s_mat3 = scalem(0.2, 0.2, 0.2); // Scaling factor for the 3rd planet
  let rotationSpeed3 = 0.8; // Adjust the rotation speed for the 3rd planet
  mvm4 = mult(mvm, t_mat3);
  mvm4 = mult(mvm4, s_mat3);
  mvm4 = mult(mvm4, rotate(theta * rotationSpeed3, rotationfactor));
  gl.uniformMatrix4fv(modelViewMatrix, false, flatten(mvm4));
  gl.drawArrays(gl.TRIANGLES, 0, points.length);
