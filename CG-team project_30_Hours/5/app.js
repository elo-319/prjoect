...
// Javidan - added more rotation due to views
...
function render() { 
    ...
    let rotationfactor = []; // rotation factor for y due to eye value

    
    if (DifferentViews === 'a') {
     
      rotationfactor = [0, 1, 0]; // we use y for top rotation
     } else if (DifferentViews === 'd') {
    
      rotationfactor = [0, 1, 0]; // we use x but -x cause eye is also -x
     } else if (DifferentViews === 'q') {
    
       rotationfactor = [1, 0, 0]; // we use x but -x cause eye is also -x
     } else if (DifferentViews === 'e') {
    
       rotationfactor = [1, 0, 0]; // we use x but -x cause eye is also -x
     } else if (DifferentViews === 'z') {
    
        rotationfactor = [0, 0, 1]; // we use x but -x cause eye is also -x
      } else if (DifferentViews === 'c') {
